...
`ChainedTranslationEngine` tries in order: 1) `DictionaryTranslationEngine` (8 hardcoded phrases, instant) → 2) `IndicTransOnnxTranslationEngine` (IndicTrans2 320M distilled, arbitrary text, **Hindi ↔ Marathi only for now** -- see below). Same language → no-op. Both fail → `TranslationUnavailableException` → UI message. Never feed Hindi text to a Tamil TTS voice.

**IndicTrans2 known limitation:** the model pivots every Indic sentence through Devanagari internally (confirmed from AI4Bharat's own `IndicTransToolkit` reference code) -- text is transliterated script → Devanagari before encoding and Devanagari → target script after decoding. That transliteration layer is not implemented on-device, so `IndicTransOnnxTranslationEngine` refuses every language pair except Hindi↔Marathi (both already Devanagari) rather than silently returning the right words in the wrong script -- reproduced directly: hin_Deva → ben_Beng came back as correct Bengali *content* spelled in Devanagari letters. English is excluded too: this checkpoint (`indic-indic-dist-320M`) translates eng_Latn → hin_Deva correctly but garbles the reverse direction, which needs AI4Bharat's separate `indic-en` checkpoint instead. Every other pair falls through to the 8-phrase dictionary or the untranslated-original fallback.
