...
- Translation: `ChainedTranslationEngine` → `DictionaryTranslationEngine` (8 phrases, instant) → `IndicTransOnnxTranslationEngine` (IndicTrans2 200M, arbitrary text). `BpeTokenizer` handles SentencePiece BPE tokenization.
...
Run `python export_translation_models.py` from the repo root to download IndicTrans2 200M, export encoder + decoder to ONNX, quantize to INT8, and save vocab JSON. Requires: `pip install transformers optimum onnxruntime sentencepiece protobuf`. Output lands in `app/src/main/assets/models/translation/`.
