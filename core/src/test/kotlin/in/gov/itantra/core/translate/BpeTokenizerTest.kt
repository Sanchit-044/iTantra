...
class BpeTokenizerTest {
    ...
    fun `encode single known word`() {
        ...
        assertTrue("Should produce non-empty IDs", ids.isNotEmpty())
    }
    ...
    fun `unknown characters map to UNK`() {
        ...
        assertTrue("Unknown chars should map to unkId", ids.any { it == tok.unkId })
    }
    ...
    fun `floresToCode maps all Languages`() {
        for (lang in Language.entries) {
            ...
            assertNotNull("$lang should have a FLORES code", code)
            assertTrue("FLORES code should contain underscore", code.contains("_"))
        }
    }
    ...
    fun `multi-word string produces multiple word-boundary markers`() {
        ...
        assertTrue("Multi-word should produce multiple IDs", ids.size > 2)
    }
}
